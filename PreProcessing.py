...
def remove_blocked_rxn(model):
    ...
    blocked_rxn = cobra.flux_analysis.variability.find_blocked_reactions(model, reaction_list=None, zero_cutoff=1e-09, open_exchanges=False)
...
def essential_reaction(model):
    ...
    for  i in model.reactions:
        ...
        if (solutionKO_i.f < 0.01) or (np.isnan(solutionKO_i.f)) :
